package com.example.smsdetection.model
...
import com.example.smsdetection.MLCEngine
import com.example.smsdetection.OpenAIProtocol
import com.example.smsdetection.OpenAIProtocol.ChatCompletionMessage
...
//    private val appDirFile = "/storage/emulated/0/Android/data/ai.mlc.mlcchat/files/"
    private val gson = Gson()
    private val modelIdSet = emptySet<String>().toMutableSet()
//    /storage/emulated/0/Android/data/ai.mlc.mlcchat/files/Qwen2.5-0.5B-Instruct-q4f16_1-MLC
    companion object {
    ...
    private fun loadAppConfig() {
        ...
        Log.d("DEBUG", "loadAppConfig: $appConfigFile")
        val jsonString: String = if (!appConfigFile.exists()) {
            Log.d("DEBUG", "loadAppConfig: appConfigFile.exists")
            application.assets.open(AppConfigFilename).bufferedReader().use { it.readText() }
        } else {
            Log.d("DEBUG", "loadAppConfig: appConfigFile not exists")
            appConfigFile.readText()
        }
        Log.d("DEBUG", "loadAppConfig: $jsonString")
        ...
        for (modelRecord in appConfig.modelList) {
            ...
            Log.d("DEBUG", "loadAppConfig: $modelConfigFile")
            if (modelConfigFile.exists()) {
                ...
                Log.d("DEBUG", "loadAppConfig: addModelConfig success!")
            } else {
//                downloadModelConfig(
...
//                    true
//                )
                Log.d("DEBUG", "loadAppConfig: model path not exist!")
            }
        }
    }
    ...
    inner class ChatState{
        ...
        private fun interruptChat(prologue: () -> Unit, epilogue: () -> Unit) {
            ...
            require(interruptable())
            Log.d("DEBUG", "interruptChat: enter")
            if (modelChatState.value == ModelChatState.Ready) {
                Log.d("DEBUG", "interruptChat: ready")
                prologue()
                epilogue()
            } else if (modelChatState.value == ModelChatState.Generating) {
                Log.d("DEBUG", "interruptChat: generating")
                prologue()
                executorService.submit {
        ...
        fun requestReloadChat(modelConfig: ModelConfig, modelPath: String) {
            ...
            Log.d("DEBUG", "requestReloadChat: enter")
            require(interruptable())
            Log.d("DEBUG", "before interruptChat")
            interruptChat(
                prologue = {
                    switchToReloading()
                },
        ...
        private fun mainReloadChat(modelConfig: ModelConfig, modelPath: String) {
            ...
            executorService.submit {
                ...
                if (!callBackend {
                        Log.d("DEBUG", "mainReloadChat: in callBackend")
                        engine.unload()
                        engine.reload(modelPath, modelConfig.modelLib)
                    }) return@submit
                Log.d("DEBUG", "mainReloadChat: after callBackend")
                viewModelScope.launch {
                    Toast.makeText(application, "Ready to chat", Toast.LENGTH_SHORT).show()
                    switchToReady()
                }
            }
            Log.d("DEBUG", "mainReloadChat: out")
        }

        private val prompt1 = "你是一个反诈领域的专家，你会接收到一段短信息，请你判断该信息是否为诈骗短信，返回'是'或者'否'。"
        private val prompt2 = "你是一个反诈领域的专家，你会收到一条诈骗短信，请你描述该信息可能属于的诈骗类别并阐述其诈骗套路。" +
                    "(诈骗类别主要有：刷单返利类诈骗，虚假网络投资理财类诈骗，虚假网络贷款类诈骗，冒充电商物流客服类诈骗，冒充公检法类诈骗，" +
                    "虚假征信类诈骗，虚假购物、服务类诈骗，冒充领导、熟人类诈骗，网络游戏产品虚假交易类诈骗，婚恋、交友类诈骗)"

        // 核心函数***
        fun myChat(content: String, detail: Boolean, callback: ChatCallback){
            require(chatable())
            switchToGenerating()
//             用于获得submit返回值
//            val future = CompletableFuture<String>()
            ...
            executorService.submit {
                historyMessages.add(ChatCompletionMessage(
                    ...
                    content = if(detail) prompt2 else prompt1
                ))
                ...
                viewModelScope.launch {
                    ...
                    for (res in responses) {
                        if (!callBackend {
                            Log.d("DEBUG", "requestGenerate: $res")
                    ...
                    Log.d("DEBUG", "after launch myChat: $streamingText")

                    if (modelChatState.value == ModelChatState.Generating) {
                        Log.d("DEBUG", "in change model state")
                        switchToReady()
//                        future.complete(streamingText)
                        callback.onResult(streamingText)
                    }
                }
            }
        }
        ...
        fun requestGenerate(prompt: String) {
            ...
            executorService.submit {
                ...
                viewModelScope.launch {
                    ...
                    var finishReasonLength = false
                    var streamingText = ""
                    Log.d("DEBUG", "requestGenerate: ${responses.toString()}")
